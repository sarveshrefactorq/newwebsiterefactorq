    ...
    configureServer(server) {
      server.middlewares.use('/api/contact', async (req, res, next) => {
        ...
        if (!smtpHost || !smtpUser || !smtpPass || !smtpFrom) {
          ...
          res.end(JSON.stringify({ ok: false, error: 'SMTP configuration is missing.' }))
          return
        }
        ...
        const html = `
          ...
          <p><strong>Interest:</strong> ${escapeHtml(payload.interest || 'Not provided')}</p>
        ...
        try {
          await transporter.sendMail({
            ...
            subject: `RefactorQ enquiry: ${payload.interest || 'General enquiry'}`,
            text,
            html,
          })
          ...
          console.error('Failed to send contact email from Vite middleware', error)
          res.statusCode = 500
          res.setHeader('Content-Type', 'application/json')
